(function() {
  ...
  getSchedule = function(uid, cb) {
    return FB.api({
      ...
      for (_i = 0, _len = resp.length; _i < _len; _i++) {
        status = resp[_i];
        handleMessage(uid, status.message);
      }
      if (cb) {
        cb();
      }
  ...
  handleMessage = function(uid, msg) {
    var c, i, item, items, line, lines, name, num, nums, tags, _i, _len, _results;
    log("" + names[uid] + " - " + msg);
    lines = (function() {
    ...
    if (5 < items.length) {
      ...
      for (_i = 0, _len = items.length; _i < _len; _i++) {
        ...
        name = item[1];
        tags = item[0];
        _results.push((num = tags.match(/\d+/)) ? (tags = tags.replace(/\s?\d+\s?/, ' ').replace(/^\s+|\s+$/g, ''), tags = num[0] + ' ' + tags, classify(item[1], tags.split(' '), uid)) : void 0);
      }
      return _results;
    }
  };
