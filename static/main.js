...
var $, X, anim_state, change_annual, checkFriendship, classify, completed, coreParse, countmutual, current_status, direct_friend, education_info, filter, find_position, friends, getFriends, getSchedule, gotSchedule, handleMessage, last_popup, lookup_user, main_search, old_thresh, people_info, placeholder_classes, processSearch, progressAnimation, progress_value, race, recognize_classes, royksopp, searchClasses, selectors, setProgress, showMutualCount, showMutualMessage, showclass, showsched, showuser, t, uploadClasses,
    ...
    return FB.api('/me', function(resp) {
      ...
      return getSchedule(me.id, function(classes) {
        if (classes.length > 0) {
          ...
          return lookup_user(me.id, function(merp) {
            if (merp.name) {
              gotSchedule(me.id, [
                {
                  message: merp.status,
...
main_search = function() {
  var cb1, cb2, teacher, time, xhr, _i, _len, _ref, _ref1, _ref2;
  ...
  xhr.onreadystatechange = function() {
    var school_name, school_year, this_year, _ref3;

    if (xhr.readyState === 4) {
      if (xhr.responseText === 'narp') {
        _ref3 = education_info(me.id), school_name = _ref3[0], school_year = _ref3[1];
        this_year = (new Date).getFullYear();
        if (this_year === parseInt(school_year) - 1) {
          $('next_year').checked = false;
        }
      }
      return change_annual();
    }
  };
  return xhr.send();
};
...
education_info = function(uid) {
  var info, place, school, school_year, sname, syear, _i, _len, _ref, _ref1, _ref2;
  ...
  if (info.education) {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      place = _ref[_i];
      sname = place != null ? (_ref1 = place.school) != null ? _ref1.name : void 0 : void 0;
      syear = place != null ? (_ref2 = place.year) != null ? _ref2.name : void 0 : void 0;
...
uploadClasses = function() {
  var cls, dense, friend, info, location, school, school_year, uid, xhr;
...
coreParse = function(status) {
  ...
  lines = (function() {
    ...
    for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
      line = _ref1[_i];
      _results.push([
        (' ' + line + ' ').toLowerCase().replace(/\ (period|pd|p.d)\ /gi, ' ').replace(/[a-z]+\?/gi, '').replace(/[^a-z0-9\s]/gi, ' ').replace(/[a-z](\d+)/i, '$1').replace(/(\d+)(st|nd|th|rd)/i, ' $1 ').replace(/\s+/g, ' ').replace(/\s[a-z]\s/g, ' ').replace(/^\s+|\s+$/g, '').replace(/\s(i+)\s/g, function(a, b) {
          return " " + b.length + " ";
  ...
  for (_i = 0, _len = items.length; _i < _len; _i++) {
    ...
    if (last && __indexOf.call("now,is,so,love,truth,go,yet,like,god,faith,teeth,fine,also,beautiful,tomorrow,awesome,bible,lord,jesus".split(','), last) >= 0) {
      return [];
    }
  }
  return items;
...
this.expand_user = function(uid, el) {
  return lookup_user(uid, function(student) {
    if (student.name) {
      student.uid = uid;
...
showsched = function(student, el) {
  ...
  if ((direct_friend[student.uid] || student.uid === me.id) && student.status_id) {
...
showuser = function(status) {
  ...
  if ((direct_friend[uid] || uid === me.id) && status.status_id) {
...
(function() {
  var date, dayname, i, name, now, _i, _results;
  ...
  for (i = _i = 1; _i <= 14; i = ++_i) {
    ...
    _results.push($('reminder').appendChild(new Option(name, i)));
  }
  return _results;
})();
...
royksopp = function() {
  ...
  return xhr.onload = function() {
    return setTimeout(function() {
      return $("checkmark").style.display = '';
    }, 300);
  };
};

change_annual = function() {
  var days, xhr;
  ...
  return xhr.send("uid=" + me.id + "&days=" + days + "&purpose=annual");
};
...
recognize_classes = function() {
  ...
  if (recognized.length > 3) {
    ...
    return $('reminderbox').style.display = 'none';
  } else {
    $('continuebox').style.display = 'none';
    return $('reminderbox').style.display = '';
  }
};
